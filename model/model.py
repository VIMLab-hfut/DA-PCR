...
class Model(nn.Module):
    ...
    def __init__(self, configuration, num_pts=2048, adain=True):
        ...
        self.adain_128 = AdaIn1dUpd(self.model_dim, num_latent=self.num_latent)
        self.adain_256 = AdaIn1dUpd(self.model_dim, num_latent=self.num_latent)
        ...
        self.cic = CIC(npoint=2048, radius=0.05, k=20, in_channels=128, output_channels=256, bottleneck_ratio=2,
                       mlp_num=1, curve_config=[100, 5])
        self.cic = CIC(npoint=2048, radius=0.05, k=20, in_channels=256, output_channels=256, bottleneck_ratio=2,
                       mlp_num=1, curve_config=[100, 5])
    ...
    def forward(self, x, noise, img_flag=True):
        if img_flag:
            ...
            # a = x
            x = self.adain_128(noise_feat, x)  # 风格迁移特征
            _, x = self.cic(noise, x)
            x = self.adain_256(noise_feat, x)  # 风格迁移特征
            _, x = self.cic2(noise, x)
